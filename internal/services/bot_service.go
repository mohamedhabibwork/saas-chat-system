...
func NewBotService(hub *websocket.Hub, db Database) *BotService {
	return &BotService{
		hub: hub,
		db:  db,
	}
...
func (s *BotService) checkSubscriptionLimits(userID int) error {
	...
	usage, err := s.getUserUsage(subscription.ID)
	if err != nil {
		return fmt.Errorf("failed to get usage: %v", err)
	}

	// Check limits
	if usage.MessagesSent >= subscription.Plan.Limits.MaxMessagesPerDay {
		return fmt.Errorf("daily message limit exceeded")
	}

	if usage.TokensUsed >= subscription.Plan.Limits.MaxTokensPerMonth {
